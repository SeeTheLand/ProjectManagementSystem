...
public interface DeveloperDAO extends DAO<Developer, Long> {
    ...
    String GET_DEVELOPERS_ON_PROJECT = "SELECT * FROM developers d INNER JOIN developers_projects dp on d.id = dp.developer_id INNER JOIN  projects p on dp.project_id = p.id WHERE p.id = ?";
}
